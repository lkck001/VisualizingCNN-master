...
def store(model):
    ...
    def hook(module, input, output, key):
        if isinstance(module, nn.MaxPool2d):
            # MaxPool2d with return_indices=True returns tuple: (output, indices)
            model.feature_maps[key] = output[0]  # Store activation maps
            model.pool_locs[key] = output[1]     # Store pooling indices for deconv
        
        # Example shapes:
        # output[0]: [1, 64, 112, 112] - Feature maps after pooling
        # output[1]: [1, 64, 112, 112] - Indices of max values for unpooling
        else:
            # For Conv2d and ReLU layers, just store the output
            model.feature_maps[key] = output
            # Example shape for Conv: [1, 64, 224, 224]
...
def vis_layer(layer, vgg16_conv, vgg16_deconv):
    ...
    choose_map = torch.where(
        choose_map == max_activation, choose_map, torch.zeros(choose_map.shape)
    )
